...
\begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
        ...
        \includegraphics[height=0.4\textheight]{report/1/apartments.png}\\
        \footnotesize Fig.: Apartment Locations
    \end{column}
...
\begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
        ...
        \begin{itemize}
            ...
            \item Reveals primary traffic routes and connectivity patterns
...
\begin{frame}{Conclusions: Visualization Strengths}
\begin{itemize}
    \item \textbf{Comprehensive coverage:} Suite of visualizations addresses all aspects of urban mobility and activity patterns
    \item \textbf{Interactive exploration:} Time filtering, geographic selection, and dynamic controls enable deep analysis
    \item \textbf{Multi-scale analysis:} From individual routines to city-wide patterns across temporal and spatial dimensions
    \item \textbf{Complementary views:} Each visualization provides unique insights while supporting findings from others
\end{itemize}
\end{frame}

\begin{frame}{Conclusions: Limitations and Key Findings}
\textbf{Limitations:}
\begin{itemize}
    \item Limited to volunteer data - may not fully represent entire population
    \item Temporal gaps in data collection could affect seasonal comparisons
    \item Spatial aggregation may obscure micro-level patterns
\end{itemize}
...
\textbf{Key Findings:}
\begin{itemize}
    \item Clear distinction between residential, commercial, and mixed-use areas
    \item Traffic patterns show strong temporal variations (weekday vs. weekend)
    \item Individual routines reveal diverse lifestyle patterns
    \item Seasonal changes indicate weather-influenced behavior shifts
\end{itemize}
\end{frame}
